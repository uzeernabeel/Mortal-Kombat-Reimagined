...
public class KabalPower2 : MonoBehaviour {
    ...
    void Update() {
        ...
        if (moveRight)
            barakaPowerPosition = new Vector2(transform.position.x, transform.position.y);
        else
            barakaPowerPosition = new Vector2(transform.position.x, transform.position.y);
        ...
        if (time > bulletTime && !gotHitFirstTime) {
            ...
            if (moveRight) {
                barakaPowerPrefab.GetComponent<KabalBullet2>().facingLeft = false;
                //Debug.Log("Some how I ended up here!");
            } else {
                barakaPowerPrefab.GetComponent<KabalBullet2>().facingLeft = true;
