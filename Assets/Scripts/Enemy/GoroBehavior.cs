...
public class GoroBehavior : MonoBehaviour {
	...
	void Update () {
        ...
        if (power1Active) {
            ...
            if (time > bulletTime && playerHitMe <= 5) {
                ...
                if (moveRight) {
                    sheevaBulletPrefab.GetComponent<GoroBullet>().facingLeft = false;
                    //Debug.Log("Some how I ended up here!");
                } else {
                    sheevaBulletPrefab.GetComponent<GoroBullet>().facingLeft = true;
