...
public class KabalPower : MonoBehaviour {
    ...
    void Start() {
        //bonesPrefabs = GameObject.FindGameObjectsWithTag("Bones");
    ...
    void Update() {
        ...
        //power coming out refrence to player direction
        if (moveRight)
            barakaPowerPosition = new Vector2(transform.position.x + 0.5f, transform.position.y + 1f);
        else
            barakaPowerPosition = new Vector2(transform.position.x - 0.5f, transform.position.y + 1f);
        ...
        if (time > bulletTime && !gotHitFirstTime) {
            ...
            if (moveRight) {
                barakaPowerPrefab.GetComponent<KabalBullet>().facingLeft = false;
                //Debug.Log("Some how I ended up here!");
            } else {
                barakaPowerPrefab.GetComponent<KabalBullet>().facingLeft = true;
