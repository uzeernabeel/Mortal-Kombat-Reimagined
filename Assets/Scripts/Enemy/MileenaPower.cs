...
public class MileenaPower : MonoBehaviour {
    ...
    void Update() {
        ...
        if (moveRight)
            barakaPowerPosition = new Vector2(transform.position.x, transform.position.y + 1.25f);
        else
            barakaPowerPosition = new Vector2(transform.position.x, transform.position.y + 1.25f);
        ...
        if (time > bulletTime && !gotHitFirstTime) {
            ...
            if (moveRight) {
                barakaPowerPrefab.GetComponent<MileenaBullet>().facingLeft = false;
                //Debug.Log("Some how I ended up here!");
            } else {
                barakaPowerPrefab.GetComponent<MileenaBullet>().facingLeft = true;
