...
public class ShangTsungPower : MonoBehaviour {
    ...
    void Update() {
        ...
        if (moveRight)
            barakaPowerPosition = new Vector2(transform.position.x - 2.5f, transform.position.y + 1f);
        else
            barakaPowerPosition = new Vector2(transform.position.x + 2.5f, transform.position.y + 1f);
        ...
        if (time > bulletTime && !gotHitFirstTime) {
            ...
            if (moveRight) {
                barakaPowerPrefab.GetComponent<ShangTsungBullet>().facingLeft = false;
                //Debug.Log("Some how I ended up here!");
            } else {
                barakaPowerPrefab.GetComponent<ShangTsungBullet>().facingLeft = true;
    ...
    private void Died() {
        //collider2d.size = new Vector2(collider2d.size.x - 0.15f, collider2d.size.y);
