...
public class KanoPower2 : MonoBehaviour {
    ...
    void Update() {
        ...
        if (moveRight)
            barakaPowerPosition = new Vector2(transform.position.x - 6.25f, transform.position.y + 1.1f);
        else
            barakaPowerPosition = new Vector2(transform.position.x + 6.25f, transform.position.y + 1.1f);
        ...
        if (time > bulletTime && !gotHitFirstTime) {
            ...
            if (moveRight) {
                barakaPowerPrefab.GetComponent<KanoBullet2>().facingLeft = false;
                //Debug.Log("Some how I ended up here!");
            } else {
                barakaPowerPrefab.GetComponent<KanoBullet2>().facingLeft = true;
