...
public class SindelPower : MonoBehaviour {
    ...
    void Update() {
        ...
        if (moveRight)
            barakaPowerPosition = new Vector2(transform.position.x, transform.position.y + 1f);
        else
            barakaPowerPosition = new Vector2(transform.position.x, transform.position.y + 1f);
        ...
        if (time > bulletTime && !gotHitFirstTime) {
            ...
            if (moveRight) {
                barakaPowerPrefab.GetComponent<SindelBullet>().facingLeft = false;
                //Debug.Log("Some how I ended up here!");
            } else {
                barakaPowerPrefab.GetComponent<SindelBullet>().facingLeft = true;
